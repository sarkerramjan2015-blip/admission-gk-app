...
# Add BDTopic13Seed to subtopics list
...
# Add BDTopic13Seed to MCQs list
old_mcqs = 'bdMcqs.addAll(BDTopic12Seed.getMCQs())'
new_mcqs = 'bdMcqs.addAll(BDTopic12Seed.getMCQs())\n                bdMcqs.addAll(BDTopic13Seed.getMCQs())'
content = content.replace(old_mcqs, new_mcqs)

# Comment out old bd_13 mangled subtopics
for i in range(1, 6):
    content = content.replace(f'GKSubTopicEntity("bd_13_{i}"', f'// GKSubTopicEntity("bd_13_{i}"')
