...
class GKRepository(private val gkDao: GKDao) {
    ...
    fun getAllSubTopics(): Flow<List<GKSubTopicEntity>> = gkDao.getAllSubTopics()
    suspend fun getAllSubTopicsSync(): List<GKSubTopicEntity> = gkDao.getAllSubTopicsSync()
    suspend fun getAllMainTopicsSync(): List<GKMainTopicEntity> = gkDao.getAllMainTopicsSync()
    suspend fun getAllMCQsSync(): List<MCQQuestionEntity> = gkDao.getAllMCQsSync()
    fun getMCQsForSubTopic(subTopicId: String): Flow<List<MCQQuestionEntity>> = gkDao.getMCQsForSubTopic(subTopicId)
    suspend fun getMCQsForSubTopicSync(subTopicId: String): List<MCQQuestionEntity> = gkDao.getMCQsForSubTopicSync(subTopicId)
    ...
    fun getMegaQuizSubTopics(examId: String): Flow<List<GKSubTopicEntity>> = gkDao.getMegaQuizSubTopics(examId)
